import psutil  #Used for system monitoring (CPU usage, process information).
import time
...
def monitor_cpu(sender_email, receiver_email, plain_password):
    ...
    try:
        ...
        while True:
            ...
            if cpu_usage > 60:
                top_process = max(psutil.process_iter(["pid", "name", "cpu_percent"]),  #iterates through all running process
                                  key=lambda p: p.info["cpu_percent"])   #find the process process with highest cpu usgae
